...
def main():
    ...
    while True:
        ...
        if lang_choice == '1':
            clear()
            print("Program Konversi Video H.264 ke H.266\n")
            input_text = "Masukkan nama file input (H.264): "
            output_text = "Masukkan nama file output (H.266): "
            done_text = "\nKonversi video selesai!"
            exit_text = "Tekan ENTER untuk keluar."
            break
        elif lang_choice == '2':
            clear()
            print("H.264 to H.266 Video Conversion Program\n")
            input_text = "Enter input file name (H.264): "
            output_text = "Enter output file name (H.266): "
    ...
    # Mengkonversi video menggunakan format H.266 (AV1)
    stream = ffmpeg.output(stream, output_file, vcodec='libsvtav1')
